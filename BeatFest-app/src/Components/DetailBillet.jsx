...
function DetailBillet() {
  ...
  const [nombreJours, setNombreJours] = useState(1); // Nouvel état pour le nombre de jours
  const [descriptionButton, setDescriptionButton] = useState('+');
  const [descriptionVisible, setDescriptionVisible] = useState(false);
  const [showPopup, setShowPopup] = useState(false); // Popup pour ajout au panier
  const [showQuantityPopup, setShowQuantityPopup] = useState(false); // Popup pour quantité nulle
  ...
  const fraisDeServiceParBillet = billet ? (billet[`prix_${nombreJours}jours`] * 0.05) : 0; // Utilisation de nombreJours
  const fraisDeService = quantite * fraisDeServiceParBillet;
  const total = billet ? (billet[`prix_${nombreJours}jours`] * quantite) + fraisDeService : 0; // Utilisation de nombreJours
  ...
  const ajouterAuPanier = () => {
    ...
    if (quantite === 0) {
      ...
      if (quantiteTotale + quantite > 8) {
        ...
        nouveauxArticles[articleExistantIndex] = { ...articleExistant, quantite: nouvelleQuantite };
        modifierPanier(nouveauxArticles); 
        setShowPopup(true);
        setTimeout(() => {
          setShowPopup(false);
        }, 2000);
      ...
      {billet ? (
        <div>
          <div className='flex space-x-1 pl-10 pt-5 text-white text-2xl'>
              ...
              <a href='/billetterie' title='Billetterie'>Billetterie</a><p>/</p>
              <h2>{billet.name}</h2>
          </div>
          <div className='p-10 flex'> 
            ...
            <div className="p-10 mr-20 max-w-md">
                ...
                <h2 className='text-2xl text-white'>{billet[`prix_${nombreJours}jours`]}€</h2> {/* Utilisation de nombreJours */}
                <div className="mt-4">
            ...
            <div className="bg-gray-200 rounded-lg h-5/6">
                ...
                <div className="bg-zinc-300">
                    ...
                    <h2 className='pl-5 pb-5'>{billet[`prix_${nombreJours}jours`]}€ + frais de billetterie</h2> {/* Utilisation de nombreJours */}
                </div>
