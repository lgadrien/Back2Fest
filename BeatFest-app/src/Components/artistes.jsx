...
const artistesData = [
    ...
    { id: 6, nom: 'Nekfeu', photoUrl: 'https://i.pinimg.com/1200x/36/2e/3a/362e3ae7d6b99932d25d9999d06113d9.jpg', description: "" },
];
...
const Calendar = () => {
    ...
    useEffect(() => {
        document.title = 'Beat Fest - Artistes';
      }, []);
    ...
    return (
        <div className='artistes-container'>
            ...
            <h1 className='mt-5 pb-5 text-white'>Programmation des artistes - 18 au 20 Juillet 2025</h1>
                    ...
                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pb-10'>
                        {dates.map(date => (
                            <div key={date} className='date-column ms-12 mt-5'>
                                <h2 className='text-center font-bold text-xl mb-4 text-white'>{date}</h2>
                                {schedule[date].map(artiste => (
                                    <div key={artiste.id} className='artiste mb-6'>
                                        ...
                                        {visible[artiste.id] && <h3 className="nom-artiste text-center text-white">{artiste.nom}</h3>}
